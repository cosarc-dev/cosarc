...
class NutritionServiceV2 {
  ...
  List<Map<String, dynamic>> _searchIndianDatabase(String query) {
    ...
    final indianFoods = <Map<String, dynamic>>[
      ...
      {
        'name': 'Dominos Margherita Regular',
        'brand': 'Dominos India',
        'calories': 265.0,
        'protein': 11.0,
        'carbs': 33.0,
      ...
      {
        'name': 'Dominos Farmhouse Regular',
        'brand': 'Dominos India',
        'calories': 227.0,
        'protein': 9.0,
        'carbs': 30.0,
      ...
      {
        'name': 'Subway Veggie Delite 6inch',
        'brand': 'Subway India',
        'calories': 230.0,
        'protein': 9.0,
        'carbs': 44.0,
    ...
    return indianFoods.where((food) {
      ...
      return name.contains(q) ||
          ...
          q
              .split(' ')
              .any((word) => name.contains(word) || brand.contains(word));
    }).toList();
  }
  ...
  Future<List<Map<String, dynamic>>> _fetchOpenFoodFacts(String query) async {
    try {
      ...
      return products
          ...
          .where((item) => item != null && item['calories'] > 0)
  ...
  Future<List<Map<String, dynamic>>> _fetchUSDA(String query) async {
    ...
    try {
      final url = Uri.parse('https://api.nal.usda.gov/fdc/v1/foods/search?'
  ...
  Future<List<Map<String, dynamic>>> _fetchSpoonacular(String query) async {
    ...
    try {
      final url =
          Uri.parse('https://api.spoonacular.com/food/ingredients/search?'
  ...
  /// ═══════════════════════════════════════════════════════════════════
  Future<List<Map<String, dynamic>>> _fetchFruitsAndVeggies(
      String query) async {
  ...
  /// ═══════════════════════════════════════════════════════════════════
  List<Map<String, dynamic>> _deduplicateResults(
  ...
  /// ═══════════════════════════════════════════════════════════════════
  List<Map<String, dynamic>> _applySmartRanking(
    ...
    items.sort((a, b) {
      ...
      if (indianBrands.any(
          (brand) => brandA.contains(brand) || a['category'] == 'indian')) {
        scoreA += 400;
      }
      if (indianBrands.any(
          (brand) => brandB.contains(brand) || b['category'] == 'indian')) {
        scoreB += 400;
      }

      // 4. FRESH PRODUCE (350 points)
      if (a['category'] == 'fruit' || a['category'] == 'vegetable') {
        scoreA += 350;
      }
      if (b['category'] == 'fruit' || b['category'] == 'vegetable') {
        scoreB += 350;
      }
      ...
      if (queryWords
          .every((word) => nameA.contains(word) || brandA.contains(word))) {
        scoreA += 300;
      }
      if (queryWords
          .every((word) => nameB.contains(word) || brandB.contains(word))) {
        scoreB += 300;
      }

      // 6. POPULAR FAST FOOD (200 points)
      final fastFoodChains = [
        'mcdonalds',
        'kfc',
        'dominos',
        'burger king',
