...
class NutritionService {
  ...
  Future<List<Map<String, dynamic>>> searchFood(String query) async {
    ...
    combined.sort((a, b) {
      ...
      bool aIsMeal = mealKeywords
          .any((k) => a['name'].toString().toLowerCase().contains(k));
      bool bIsMeal = mealKeywords
          .any((k) => b['name'].toString().toLowerCase().contains(k));
  ...
  Future<List<Map<String, dynamic>>> _fetchOFF(String query) async {
    try {
      final url = Uri.parse(
          'https://world.openfoodfacts.org/cgi/search.pl?search_terms=$query&search_simple=1&action=process&json=1&page_size=30&cc=in');
  ...
  Future<List<Map<String, dynamic>>> _fetchUSDA(String query) async {
    try {
      final url = Uri.parse(
          'https://api.nal.usda.gov/fdc/v1/foods/search?query=$query&pageSize=20&api_key=$_usdaKey');
      final res = await http.get(url).timeout(const Duration(seconds: 5));
      if (res.statusCode != 200) return [];
...
class _NutritionScreenState extends State<NutritionScreen> {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text('FUEL LOG',
            style: TextStyle(
                letterSpacing: 4, fontWeight: FontWeight.w900, fontSize: 13)),
        centerTitle: true,
      ...
      body: ValueListenableBuilder(
        ...
        builder: (context, Box<FoodLog> box, _) {
          ...
          final logs = box.values
              .where((l) =>
                  l.dateTime.day == now.day && l.dateTime.month == now.month)
              .toList();
          ...
          return CustomScrollView(
            ...
            slivers: [
              ...
              _mealGroup(
                  "Snacks", logs.where((l) => l.mealType == "Snack").toList()),
    ...
    return Container(
      ...
      child: Column(children: [
        Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
          _statCol("REMAINING", rem < 0 ? "0" : rem.toInt().toString(),
              cosarcAccent),
        ...
        ClipRRect(
            ...
            child: LinearProgressIndicator(
                value: (c / dailyTarget).clamp(0, 1),
                minHeight: 12,
                backgroundColor: Colors.white10,
                color: cosarcPink)),
        const SizedBox(height: 25),
        Row(mainAxisAlignment: MainAxisAlignment.spaceAround, children: [
          _miniStat("P", p, Colors.blue),
          _miniStat("C", carb, Colors.orange),
          _miniStat("F", f, Colors.purple),
        ])
      ]),
    );
  }
  ...
  Widget _statCol(String l, String v, Color col) => Column(children: [
        ...
        Text(v,
            style: TextStyle(
                fontSize: 32, fontWeight: FontWeight.w900, color: col)),
      ]);
    ...
    return SliverMainAxisGroup(slivers: [
      SliverToBoxAdapter(
        ...
        child: Row(children: [
          ...
          IconButton(
              onPressed: () => _openSearch(context, name),
              icon: const Icon(Icons.add_circle_rounded,
                  color: cosarcAccent, size: 32)),
        ]),
      )),
      SliverList(
          delegate: SliverChildBuilderDelegate((_, i) => _foodTile(logs[i]),
                ...
                child: Column(
                  ...
                  Text(log.name,
                      style: const TextStyle(
                          ...
                          color: Colors.white)),
                  Text(
                      "${log.quantity.toInt()} ${log.unit} • P: ${log.protein.toInt()}g",
                      style:
  ...
  void _openSearch(BuildContext ctx, String m) => showModalBottomSheet(
      context: ctx,
      isScrollControlled: true,
      backgroundColor: Colors.transparent,
      builder: (_) => FoodSearchComponent(preSelectedMeal: m));
}
      ...
      child: Column(children: [
              ...
              Container(
                ...
                child: DropdownButtonHideUnderline(
                  child: DropdownButton<String>(
                    value: selectedUnit,
                    dropdownColor: cosarcSurface,
                    icon: const Icon(Icons.unfold_more,
                        color: cosarcPink, size: 20),
                    items: _units
        ...
        if (_loading)
          const Padding(
              ...
              child:
                  CircularProgressIndicator(color: cosarcPink, strokeWidth: 3)),
        Expanded(
              ...
              child: ListTile(
                ...
                title: Text(item['name'],
                    style: const TextStyle(
                        ...
                        fontWeight: FontWeight.bold,
                        color: Colors.white)),
                subtitle: Text("${item['brand']} • 1 Serv ≈ ${base.toInt()}g",
                    style:
                        const TextStyle(color: Colors.white38, fontSize: 12)),
                trailing: IconButton(
                  icon:
                      const Icon(Icons.add_circle, color: cosarcPink, size: 34),
                  onPressed: () {
                ...
                onTap: () =>
                    _showDetail(item, factor), // 👈 Tap card for details
              ),
            );
          },
        )),
        ...
        builder: (_) => AlertDialog(
              ...
              content: Column(
                  ...
                  children: [
                    ...
                    _detailRow("Energy",
                        "${(item['calories'] * f).toInt()} kcal", cosarcPink),
                    _detailRow(
                        "Protein",
                        "${(item['protein'] * f).toStringAsFixed(1)} g",
                        Colors.blueAccent),
                    _detailRow(
                        "Carbs",
                        "${(item['carbs'] * f).toStringAsFixed(1)} g",
                        Colors.orangeAccent),
                    _detailRow(
                        "Fat",
                        "${(item['fat'] * f).toStringAsFixed(1)} g",
                        Colors.purpleAccent),
                  ]),
            ));
  }
